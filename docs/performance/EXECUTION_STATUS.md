...
| P1-01 | NOT_STARTED | Measured and re-scoped: the fixed-trace (CSDR) route already implements the predictor/corrector split, so the plan's 1.08-1.20x CSDR target does not exist; the repeated `apply_Theta!` materialization is in the generic non-fixed-trace route only. See `docs/evidence/P1_01_SCOPING_MEASUREMENT.md`. Not implemented. |
