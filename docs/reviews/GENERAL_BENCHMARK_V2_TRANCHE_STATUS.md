...
5. Certified first tranche: six typed LP kinds (box, sparse planted KKT,
   Nonpositive sign, Chebyshev, primal-infeasible Farkas, and dual-infeasible
   improving ray), two exact ill-conditioned LPs (diagonal-scale and
   near-rank-loss), three RSOC cases (quadratic epigraph, perspective LS,
   many QR3), and three SDP cases (weighted trace, Max-Cut K4, eight PSD(3)
   multiblock) are solve-eligible with independent Float64 receipts.
   Duplicate/rank-deficient and near-boundary LPs (solver
   `numerical_breakdown`), SOCP, Hilbert-6 SDP, boundary SOC, high-range
   EXP/Power, and mixed kinds remain open rather than represented by placeholders.
