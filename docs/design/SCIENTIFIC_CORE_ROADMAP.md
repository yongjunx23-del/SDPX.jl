...
- R4 配对基准首片 `b868b20` 已实现实际四 limb 位型 fixture、独立 exact-dyadic 整数逐项参考和诚实误差指标，尚待独立审阅。Float64x4/MF3.2.6、MFLA5399c0cc、MFAcdb84680 下，1/4线程各199项通过；另一次固定(23,65,17)、paircancel、β=0.5输入/参考准备通过，三个进程各173个来源/环境散列不变。参考在2^-2148网格精确累加，不调用 BLAS/MFA/MFLA 或 MPFR 生成参考；原生输出不重归一化，3.1e-61门明确标为 max(1,‖reference‖∞) 混合尺度，并另报无floor相对误差和零分母。ABBA+BAAB实际调用、逐个计时输出保存/检查、执行回执与至少三次独立进程重复仍未实现；不提交性能campaign、不计速度资格。证据：`paired-benchmark-repair/parent-report.md`。
