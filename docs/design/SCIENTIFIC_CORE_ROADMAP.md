...
- R3：BFLA/QDLDL 0.4.1 在 n=12、256/512/1024-bit、单线程下通过 231 项父流程复测，包含原矩阵残差、所有权和失效控制；加载来源绑定 fail-closed。它尚未连接 SDPX 原生稀疏路线，也不构成规模/性能资格。
- R4：MFLA/MFA 两个尺寸已有同类型 ABBA 多进程重复观察、全幅输入和可测的产品对消；仍无通用或 solver 速度资格。ABBA 不排除全部位置效应，后续须测试实际调用路径并继续保留限定范围。
- R0：独立几何反例已永久接线。修正后的 Power 区间参考通过独立审阅及 337 项父流程复测，仅限同工作精度 BigFloat 参考，未接入生产或取得 Float64/x4 资格；完整公开套件仍受既有 Power/mixed Exp 失败阻断。
