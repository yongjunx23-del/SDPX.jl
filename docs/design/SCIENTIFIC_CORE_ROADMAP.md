...
7. **公开路由资格控制 + 审阅初检修复**：typed 数值拒绝→`NumericalFailure` Result（不再逃逸 optimize!）；诚实 ExecutionPlan 组件（formulation reason/provenance、dense LU LA 配置、`factor_pair_dense_core` 存储计划）；64 MiB 同时存活上限 + 强制；新增公开资格控制（非 half 指数/SOC/Exp/LP 形状 typed 拒绝、非有限数据、目标符号/常数/缩放、源与结果 mutation 隔离、默认路由 unchanged）。

**独立审阅（`42dc47e3`）在 Codex usage limit 处中断（基础设施阻塞，不构成通过）**；其初检项已全部修复。仍未完成：该审阅的完整闭环、完整资格矩阵、完整内存上界、Exp whole-epoch 迁移决策。公开默认 Float64 Power/Exp dispatch **未修复**（默认仍 breakdown，known-issue control 保留）。
