...
- R1：BFLA `aaa71f3` 的有效舍入与 RRQR 创建语境元数据修复通过核心源码/证据复核。父流程独立执行：Julia 1.12 核心 10,960 项，Julia 1.10 核心 10,957 项加 1 项不适用，双线程 285 项；48 组同运行时默认舍入记录一致。Julia 1.11 与可选扩展仍未验证。
...
- R4：MFLA/MFA 已有同机同类型初步测量；尚无合格速度收益。仍需完全反平衡采样、可验证的强对消构造及基于最终回执的统计。
- R0：独立几何反例已永久接线。Power 区间参考仍被舍入方向、预算计数和测试 oracle 缺陷阻断，未接入生产；完整公开套件仍受既有 Power/mixed Exp 失败阻断。
