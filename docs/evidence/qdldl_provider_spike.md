...
- The committed spike records allocation/timing evidence only for its
  dimension-16 fixture. It proves fixed-pattern refactor execution, not a
  dimension-independent allocation law. Multi-dimension measurements must be
  generated by a separate checked-in benchmark before being cited.
...
The committed test does not generate dimensions 41 or 82, so no crossover or
scaling claim is made here. Route thresholds require a separate fresh-process,
multi-dimension benchmark with fill/RSS and confidence intervals. The
production memory-budget gate (`docs/design/HIGH_PRECISION_SPARSE_PROVIDERS.md`
§Densification gate) remains authoritative.
