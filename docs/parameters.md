...
| `formulation` | `:auto` | Eligible general dense systems use the static ProblemFeatures planner to choose dense normal equations or dense augmented KKT before provider execution. `:normal_equations` and `:augmented` are expert overrides; `:primal` preserves historical structural routing; `:dual` fails closed. No runtime formulation retry is performed. |
