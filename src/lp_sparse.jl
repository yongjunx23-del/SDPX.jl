...
# Sparse LP policy and backend selection.  The policy lives beside the
# dedicated LP sparse system; SDP KKT backends do not own these decisions.
...
function select_lp_formulation(
    ...
    # BigFloat/MultiFloat, so package availability must not silently change
    # an :auto execution plan.
