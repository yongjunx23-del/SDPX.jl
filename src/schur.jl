...
function _accumulate_buildP_sparse!(
    ...
    if size(coeffs, 1) == 3
        ...
        P[1, 1] = p11
        P[1, 2] = p12
        # BigFloat is mutable: the symmetric entries must not share one MPFR
        # object when this allocation-safe public helper is called directly.
        P[2, 1] = T === BigFloat ? MA.mutable_copy(p12) : p12
        P[2, 2] = p22
        return P
    end
    blocks = cons.Asp[l]
