...
plus matching source/environment/campaign identity.  Report status
distinguishes a real test failure (FAILED marker), a missing report (no
...
def _build_row(kind, root_arg, manifest, failures):
    ...
    if not row["report_complete"]:
        failures.append(
            ...
            "present={2}, identity match={3})".format(
                label,
...
def _write_synthetic_root(
    ...
    if not missing_test_log:
        (root / "test.log").write_text("synthetic test log\n")
...
def main(argv):
    parser = argparse.ArgumentParser(
        description=(
            "Aggregate the parallel v041 canonical Stage-1 focused/full run; "
            "writes deterministic summary.json and summary.md."
        ),
    )
    parser.add_argument(
        "--manifest",
