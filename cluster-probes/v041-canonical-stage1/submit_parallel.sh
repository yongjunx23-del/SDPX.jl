#!/bin/bash
# Submit one canonical Stage-1 focused job and one full Pkg.test job to two
# distinct compute nodes concurrently, sharing one immutable campaign identity.
#
...
FULL_JOB_ID=""
if [ "$SUBMIT" = "1" ]; then
  FOCUSED_JOB_ID="$(submit_one focused "$FOCUSED_NODE" "$OUTPUT_ROOT" sdpx_canonical_stage1_focused)" || {
  ...
  FULL_JOB_ID="$(submit_one full "$FULL_NODE" "$FULL_OUTPUT_ROOT" sdpx_canonical_stage1_full)" || {
