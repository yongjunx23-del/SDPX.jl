#!/bin/bash
# Login-node bootstrap for the Stage-1 independent Julia environment.
# Only instantiates and precompiles the env named by BOOTSTRAP_ENV.  Compute
# nodes have no package network access, so this must run on a login node.
set -euo pipefail
