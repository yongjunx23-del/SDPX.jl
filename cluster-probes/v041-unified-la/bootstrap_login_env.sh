#!/bin/bash
# Login-node bootstrap for the v041-unified-la independent Julia environment.
# MultiFloatLinearAlgebra is unregistered, so it is never part of SDPX's test
# target; the caller develops the SDPX candidate and the pinned MFLA checkout
# into BOOTSTRAP_ENV before running this script.
set -euo pipefail
