#!/bin/bash
# Static preflight for the parallel focused/full parameter-provenance
# Stage-1 submission layer.  No Julia, Pkg, SSH, or qsub is executed here.
set -euo pipefail
...
for job in focused full; do
  ...
  check grep -qF 'RUNTIME_CONTRACT julia=' "$HERE/$job.pbs"
...
check grep -qF 'RUNTIME_CONTRACT julia=4 plan=4 blas=1' "$HERE/aggregate_parallel.py"
check grep -qF 'candidate_source_realpath' "$HERE/aggregate_parallel.py"
...
check grep -qF 'RUNTIME_CONTRACT' "$HERE/README.md"
check grep -qF 'candidate_source_realpath' "$HERE/README.md"
