# v041-parameter-provenance-stage1 cluster probes

Cluster-only Stage-1 correctness gate for the parameter-provenance focused
boundary and the production `AutoPlanner` planning boundary.  The PBS jobs run
...
- `focused.pbs` - `ppn=5`, Julia/solver `4` threads, BLAS/OMP/MKL/BLIS `1`;
  validates node, candidate identity and hash, asserts the runtime contract
  below, then runs the compact `executed_diagnostics.jl` +
  `auto_planner.jl` gate.  No public API changes are in scope for this
  milestone, so `public_api.jl` is intentionally not included.
- `full.pbs` - same resource and runtime contract; `Pkg.test("SDPX";
  coverage=false)` into a separate result directory.
...
- `aggregate_parallel.py` - correctness-only gate over the two result roots;
  verifies markers, runtime contract, environment, and identity, then writes
  deterministic `summary.json` and concise `summary.md`.
...
recording every failing file.  The focused matrix is exactly the two files
above; the full matrix is exactly one `Pkg.test("SDPX"; coverage=false)`.
They are intentionally not duplicated into two file lists.

## Runtime contract

Both PBS jobs fail closed before any test work unless all of the following
hold:

- `PBS_NP` is exactly `5`;
- `JULIA_NUM_THREADS` and `SDPX_SOLVER_THREADS` are exactly `4`, Julia starts
  with `-t 4`, and runtime `Threads.nthreads()` is `4`;
- `OPENBLAS_NUM_THREADS`, `OMP_NUM_THREADS`, `MKL_NUM_THREADS`, and
  `BLIS_NUM_THREADS` are exactly `1`, and runtime
  `LinearAlgebra.BLAS.get_num_threads()` is `1`;
- a default `build_execution_plan` for a tiny SDP has `plan.threads == 4`.

`environment.txt` records `runtime_contract=ok` only after the Julia process
passes the identity, thread, BLAS, and default-plan asserts and the job
verifies the focused/full markers in `test.log`.  Both Julia processes also
emit the exact line
`RUNTIME_CONTRACT julia=4 plan=4 blas=1`; the aggregation gate requires that
line in `test.log` as well.  `SUCCESS` is written only after that marker
verification.
...
1. Prepare `CANDIDATE_ENV` on a login node (dev the candidate into it and add
   ...
   bash cluster-probes/v041-parameter-provenance-stage1/bootstrap_login_env.sh
   ```
...
bash cluster-probes/v041-parameter-provenance-stage1/submit_parallel.sh \
...
```bash
python3 cluster-probes/v041-parameter-provenance-stage1/aggregate_parallel.py \
...
non-empty `SUCCESS`, no `FAILED`, `environment.txt`, `test.log`, and
`process.time.txt`; the expected focused/full markers are all present in
`test.log`; `environment.txt` records `runtime_contract=ok` and matches the
manifest on campaign id, candidate source/realpath/env/tree hash, expected
node/job id, and thread/ppn contract; and the two rows agree on shared
identity fields including `candidate_source_realpath`.  The gate is
correctness-only: no ratios, CVs, medians, or timing comparisons are made.
`FAILED` is reported as `TEST_FAILURE`; a missing `SUCCESS`/`FAILED` pair with
no timing evidence is `MISSING_REPORT`; only a `/usr/bin/time` `Command
terminated by signal` line converts that to `TIMING_NOISE`.  The summary root gets
`AGGREGATE_GATE_PASS` or `AGGREGATE_GATE_FAILED`, and
`aggregate_parallel.py --self-test` covers PASS, deterministic JSON, test
failure, timing noise, missing report, hash/job-id mismatch, missing test
log/environment, focused/full marker failure, and missing runtime contract in
...
- `environment.txt` - PBS job id, hostname/node check, Julia binary/version,
  ...
  metadata when present, Julia/solver/BLAS threads, PBS `ppn`, CPU info,
  `runtime_contract=ok`, and start/finish timestamps.
- `process.time.txt` - `/usr/bin/time -v` output, including maximum resident
  set size.
- `test.log` - focused markers (`FOCUSED_BEGIN`/`FOCUSED_END`/
  `FOCUSED_FAILED`) or full `Pkg.test` markers (`FULL_BEGIN`/`FULL_END`),
  plus the `RUNTIME_CONTRACT` line.
- `SUCCESS` - written only after every focused test (or the full suite)
  passes and the marker verification succeeds.
