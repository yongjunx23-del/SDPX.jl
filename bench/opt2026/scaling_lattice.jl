...
function main()
    ...
    open(output, "a") do io
        header && println(io, join([
                ...
                "setup_validation", "precision_preparation",
                "equilibration", "parameter_selection",
                "workspace_setup", "initialization", "initial_residual",
                "kkt_schur_copy", "kkt_schur_factorization",
                "kkt_constraint_triangular_solve", "kkt_equality_gram",
                "kkt_equality_factorization", "kkt_other",
                "predictor_rhs", "predictor_linear_solve",
                "predictor_direction_recovery",
                "complementarity_analysis", "corrector_rhs",
                "corrector_linear_solve", "refinement",
                "corrector_direction_recovery", "finalization", "other",
            ], ","))
        println(io, join([
                ...
                @sprintf("%.6f", t.setup_validation),
                @sprintf("%.6f", t.precision_preparation),
                @sprintf("%.6f", t.equilibration),
                @sprintf("%.6f", t.parameter_selection),
                @sprintf("%.6f", t.workspace_setup),
                @sprintf("%.6f", t.initialization),
                @sprintf("%.6f", t.initial_residual),
                @sprintf("%.6f", t.kkt_schur_copy),
                @sprintf("%.6f", t.kkt_schur_factorization),
                @sprintf("%.6f", t.kkt_constraint_triangular_solve),
                @sprintf("%.6f", t.kkt_equality_gram),
                @sprintf("%.6f", t.kkt_equality_factorization),
                @sprintf("%.6f", t.kkt_other),
                @sprintf("%.6f", t.predictor_rhs),
                @sprintf("%.6f", t.predictor_linear_solve),
                @sprintf("%.6f", t.predictor_direction_recovery),
                @sprintf("%.6f", t.complementarity_analysis),
                @sprintf("%.6f", t.corrector_rhs),
                @sprintf("%.6f", t.corrector_linear_solve),
                @sprintf("%.6f", t.refinement),
                @sprintf("%.6f", t.corrector_direction_recovery),
                @sprintf("%.6f", t.finalization),
                @sprintf("%.6f", t.other),
            ], ","))
    end
