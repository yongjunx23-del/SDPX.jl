...
# `benchmark/gates.jl`. Runtime is measured but not gated -- shared CI runners vary
# by two to three times, and a gate that fires on noise is a gate people learn
# to ignore. Use `julia --project=. benchmark/gates.jl --check-runtime` on a quiet
# machine for that half.
#
# When a change legitimately alters these numbers -- including improving them
# -- re-record with `julia --project=. benchmark/gates.jl --record` and commit the
# updated `benchmark/baselines/gates.json` alongside it. Recording the change is
...
@testset "acceptance gates" begin
    include(joinpath(@__DIR__, "..", "benchmark", "gates.jl"))
