...
function main(argv)
    ...
    @testset "Q02 release matrix ($tier)" begin
        ...
        @testset "record vs the working trees" begin
            ...
            # NO ASSERTION HERE, deliberately, and the absence is the honest form.
            # This line was `@test agree >= 0`, which cannot fail: `agree` is a count
            # in 0:3, so the assertion contributed a pass to the suite total while
            # checking nothing. The comment above says agreement is RECORDED rather
            # than asserted, and that is what happens now. Asserting
            # `agree == length(FIRST_PARTY)` instead would be wrong in the other
            # direction: the record is a historical fact, so a live tree ahead of it
            # is legitimate. What the comment claims IS asserted is that every repo
            # resolves and every HEAD is readable, and both are asserted in the loop
            # above (`@test q02_is_repo(repo)`, `@test head.ok`).
        ...
        @testset "gitignored Manifest.toml hashes" begin
            ...
            # `>= 0` was vacuous. What matters is that EVERY first-party Manifest was
            # actually compared: if one is missing from disk, or the record carries no
            # hash for it, the loop `continue`s and `checked` falls short. So assert
            # the count, which can fail, rather than a lower bound that cannot.
            @test checked == length(FIRST_PARTY)
        end
        ...
        @testset "capability table matches its evidence" begin
            ...
            if execute
                ...
                # Was `@test true`, which counted a check that NEVER RAN as a passing
                # assertion. That is the SKIP-as-PASS failure this packet forbids by
                # name, and it inflated this driver's headline total. A skip is
                # recorded as a skip.
                @test_skip false
            end
