...
function snapshot()
    ...
    for f in ("power_half_phi_reference.jl","POWER_HALF_PHI_REFERENCE.md","power_half_root_geometry.jl",
        "power_half_root_geometry_capture.jl","test_power_half_root_geometry.jl",
        "run_power_half_root_geometry.jl","POWER_HALF_ROOT_GEOMETRY.md","fixtures/half_root_geometry.toml")
...
try
    include("test_power_half_root_geometry.jl")
    @eval encode(x::PowerHalfRootGeometry.I)=Dict("lower"=>encode(x.lo),"upper"=>encode(x.hi))
    save("results",Dict("cases"=>encode.(ROOT_GEOMETRY_RESULTS),"truth"=>ROOT_GEOMETRY_TRUTH))
finally
    after=snapshot();save("after",after);@assert before==after
end
println("SOURCE_UNCHANGED_HALF_ROOT_GEOMETRY_CHECKS_PASSED")
