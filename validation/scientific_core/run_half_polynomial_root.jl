...
function snapshot()
    ...
    for f in ("power_half_phi_reference.jl","POWER_HALF_PHI_REFERENCE.md","power_half_root_geometry.jl",
        "half_power_polynomial_root.jl","test_half_polynomial_root.jl",
        "run_half_polynomial_root.jl","HALF_POWER_POLYNOMIAL_ROOT.md","fixtures/half_root_geometry.toml")
...
try
    include("test_half_polynomial_root.jl")
    @eval encode(x::PowerHalfRootGeometry.I)=Dict("lower"=>encode(x.lo),"upper"=>encode(x.hi))
    save("results",Dict("cases"=>encode.(POLYNOMIAL_ROOT_RESULTS)))
finally
    after=snapshot();save("after",after);@assert before==after
end
println("SOURCE_UNCHANGED_HALF_POLYNOMIAL_ROOT_CHECKS_PASSED")
