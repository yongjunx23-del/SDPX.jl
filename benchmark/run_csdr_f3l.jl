...
# Gauss-Legendre nodes & weights in [-1, 1]
...
function solve_csdr_f3l(; J::Int=40, N_mu::Int=120, obj_sense=:max, tol=1e-8, max_iter=200)
    T = Float64
    println("=================================================================")
    println("  CSDR F3L Tier (J=$J, N_mu=$N_mu, sense=$obj_sense, tol=$tol)")
    println("=================================================================")
    ...
    # The fixed-trace Lorentz block alone implies 0 ≤ q ≤ 2.  Keeping q free
    # avoids a redundant orthant block per spectral point.
    ...
    for (row_idx, cond) in enumerate(conditions)
        ...
        # LHS: sum_{p=0}^m sum_{q=0}^p binom(alpha, m-p) [-(1-a)]^{m-p} a^q c_{p,q}
        expr = Any[]
        for p in 0:m
        ...
        for (l_idx, ell) in enumerate(spins)
            for i in 1:N_mu
                ...
                # W = (omega / z^2) * 16 * (2l + 1) * D^alpha * K_m * P_l
...
if abspath(PROGRAM_FILE) == @__FILE__
    solve_csdr_f3l(J=10, N_mu=120, obj_sense=:max)
end
