...
@testset "bordered route fallback" begin
    ...
    # The degenerate duplicate has two honest platform outcomes. On macOS and
    # Windows it reaches the exact early predictor signature and exercises the
    # strict expanded terminal failure. On Linux/CHOLMOD the bordered dispatch
    # can fail earlier; in that case the exact trigger MUST NOT be invented and
    # receipts must remain bordered-only. The CI matrix collectively covers
    # both paths; neither outcome is promoted to an optimum.
    if hasproperty(duplicate_selected, :route_restart_reason)
        ...
        @test duplicate_selected.requested_kkt_formulation === :auto
        @test duplicate_selected.planned_kkt_formulation === :symmetric_augmented_hsd_core
        @test duplicate_selected.executed_kkt_formulation === :symmetric_augmented_hsd_core
        @test duplicate_selected.requested_kkt_route === :bordered
        @test duplicate_selected.planned_kkt_route === :bordered
        @test duplicate_selected.executed_kkt_route === :bordered
        @test duplicate_selected.planned_kkt_storage === :sparse
        @test duplicate_selected.executed_kkt_storage === :sparse
        @test duplicate_selected.planned_factorization === :symmetric_ldl
        @test duplicate_selected.executed_factorization === :symmetric_ldl
        @test duplicate_selected.factorization_kernel === :cholmod_symmetric_ldl
        @test duplicate_selected.planned_factorization_kernel === :cholmod_symmetric_ldl
        @test duplicate_selected.executed_factorization_kernel === :cholmod_symmetric_ldl
        @test duplicate_selected.formulation === duplicate_selected.planned_kkt_formulation
        @test duplicate_selected.route === duplicate_selected.executed_kkt_route
        @test duplicate_selected.backend === duplicate_selected.executed_backend
        @test duplicate_selected.fallback_chain === ()
        @test duplicate_selected.attempted_kkt_routes === (:bordered,)
        @test duplicate_selected.executed_fallback_chain ===
            duplicate_selected.attempted_kkt_routes
        @test duplicate_selected.planned_la_provider === :cholmod
        @test duplicate_selected.la_executed_provider === :cholmod
        @test !hasproperty(duplicate_selected, :route_restart_iteration)
    end
