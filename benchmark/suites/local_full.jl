...
const _LOCAL_FULL_PATHOLOGICAL_IDS = String[
    spec.id for spec in PATHOLOGICAL_SPECS if :local_full in spec.tiers
]

const LOCAL_FULL_SUITE = let entries = SuiteEntry[]
    for id in sort!(vcat(
        _LOCAL_FULL_PUBLIC_IDS,
        _LOCAL_FULL_SYNTHETIC_IDS,
        _LOCAL_FULL_PATHOLOGICAL_IDS,
    ))
    ...
    append!(entries, SuiteEntry[
        ...
        SuiteEntry("pathological/lp_degenerate_scaled", :float64x4, :auto),
        SuiteEntry("pathological/socp_near_tangent", :float64x4, :auto),
        SuiteEntry("pathological/sdp_small_eigenvalue", :float64x4, :auto),
        SuiteEntry("pathological/sdp_congruence_scaling", :bigfloat256, :auto),
        SuiteEntry("pathological/sdp_infeasible_margin", :bigfloat256, :auto),
